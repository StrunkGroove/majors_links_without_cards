...
from base_app.info import key_mexc, crypto_list, accept_list_mexc, rc
...
def del_fake(data):
    ...
    for i in range(len(data) - 1, -1, -1):
        ad = data[i]
        symbol = ad['symbol']
        if symbol not in accept_list_mexc:
...
def save_db(token_dict):
    time_cash = 60
    cache.set(key_mexc, token_dict, time_cash)


@app.task
def main():
    url_spot = 'https://api.mexc.com/api/v3/ticker/price'
    url_bid_ask = 'https://api.mexc.com/api/v3/ticker/bookTicker'
