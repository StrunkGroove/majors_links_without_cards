...
from base_app.info import key_gateio, crypto_list, accept_list_gateio
...
def del_fake(data):
    ...
    for i in range(len(data) - 1, -1, -1):
        ad = data[i]
        symbol = ad['currency_pair']
        if symbol not in accept_list_gateio:
    ...
    for first_token in crypto_list:
        for second_token in crypto_list:
            if first_token != second_token:
                token = first_token + '_' + second_token
                token_dict[token] = {
                    'first':first_token,
                    'second':second_token,
                    'exchange':'gateio',
...
def append_info(data, token_dict):
    for token in token_dict.copy():
        for ad in data:
            ...
            price = ad['last']
            bid_price = ad['highest_bid']
            ask_price = ad['lowest_ask']
            bid_qty = ad['base_volume']
            ask_qty = ad['quote_volume']
...
def save_db(token_dict):
    time_cash = 60
    cache.set(key_gateio, token_dict, time_cash)
